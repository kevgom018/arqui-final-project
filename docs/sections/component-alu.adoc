...
The Arithmetic Logic Unit is a combinational circuit that performs binary arithmetic and logical operations. It serves as the main computational block for data manipulation during instruction execution.

==== Specifications

The figure below shows the ALU circuit in CircuitVerse with the input and output signals labeled.
...
NOTE: The most significant bit of the `alu_ctrl` signal is treated as a don't care bit because only four operations, representable by 2 bits, are implemented.
...
The ALU testing protocol exercises all supported operations across diverse operand patterns and edge cases. Six test categories provide coverage:
...
All ALU test cases execute successfully with results matching predicted values. The ALU correctly implements addition, subtraction, AND, and OR operations with proper two's complement handling and accurate zero flag generation. The component is validated for integration into the processor datapath.
