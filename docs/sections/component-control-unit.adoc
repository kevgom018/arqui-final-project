...
The figure below shows the Control Unit circuit in CircuitVerse with the input and output signals labeled.

.Control Unit Circuit Diagram
image::./images/control_unit.png[Control Unit Circuit Diagram, pdfwidth=100%, align=center]
...
NOTE: For load (`LW`), store (`SW`), and branch-equal (`BEQ`) instruction types, the `f3` and `f7` fields do not affect the control signals in this implementation, so they are treated as don't care.
...
All test cases produced the expected control outputs. The Control Unit decoding behavior matches the test encodings and the don't-care handling for `f3`/`f7` in LW, SW, and BEQ cases. The component is validated for integration into the processor datapath.