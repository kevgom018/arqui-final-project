...
After verifying the individual components, the complete processor is validated by executing the full instruction sequence. This integration phase confirms that the components interact correctly and that the processor achieves the intended behavior when executing the program.
...
. Force clock cycles one by one and observe the processor executing the instruction sequence defined in the instruction memory from the timing diagram view, while monitoring key signals and outputs at each clock cycle.
. Compare the observed behavior against expected results based on the instruction sequence and initial processor state.

For context, the contents of the instruction memory, register file, and data memory at the start of execution are shown below:
...
NOTE: Hexadecimal values are used for clarity because the CircuitVerse simulation displays values in hex in the timing diagram and its corresponding "Flag" elements.
...
NOTE: The shift of 1 cycle of data state between our table and the timing diagram is due to the fact that data is written at the end of the clock cycle, so the observed values for x1, x2, mem[0x01], x4, and x5 reflect the updated state after the instruction execution for that cycle.
